...
onMounted(() => {
  // 检查是否已完成口试报考
  const oralExamStr = localStorage.getItem('oral_exam')
  if (!oralExamStr) {
    alert('请先完成口试报考')
  ...
  if (confirmedInfoStr) {
    ...
    // 设置口试日期（笔试日期后一天）
    const examDateParts = confirmedInfo.examDate.split(' ')
    const datePart = examDateParts[0]
    examInfo.oralExamDate = datePart.replace(/\d+日/, (match) => {
      const day = parseInt(match) + 1
      return day + '日'
    }) + ' 14:00-15:00'
    
    examInfo.examLocation = oralExam.examLocation
    examInfo.examLocationName = oralExam.examLocationName
    examInfo.examRoom = oralExam.examRoom
    examInfo.examRoomName = oralExam.examRoomName
    
    // 检查是否已缴费
    const paymentStr = localStorage.getItem('oral_payment')
    if (paymentStr) {
...
const handlePay = () => {
  ...
  if (confirm('确认支付' + examInfo.fee + '元?')) {
    ...
    setTimeout(() => {
      ...
      // 保存支付信息
      localStorage.setItem('oral_payment', JSON.stringify({
        status: 'paid',
